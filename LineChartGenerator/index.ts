/**
 * LineChartGenerator - Image-Charts PCF Component
 * @version 1.0.0
 */
...
export class LineChartGenerator implements ComponentFramework.StandardControl<IInputs, IOutputs> {
  ...
  public init(context: ComponentFramework.Context<IInputs>, notifyOutputChanged: () => void, state: ComponentFramework.Dictionary, container: HTMLDivElement): void {
    ...
    this._container.className = 'image-charts-linechart-container';
    this._imgElement = document.createElement("img");
    this._imgElement.className = 'image-charts-linechart';
  ...
  private _performUpdate(context: ComponentFramework.Context<IInputs>): void {
    ...
    const lineWidth = context.parameters.lineWidth?.raw || 2;
    ...
    const url = this._buildChartUrl({ accountId, secretKey, privateCloudDomain, data, labels, colors, title, lineWidth, chartSize, advancedOptions });
    this._signedUrl = url;
  ...
  private _buildChartUrl(params: any): string {
    ...
    const queryParts: string[] = [`cht=lc`, `chs=${params.chartSize}`, `chd=${formatDataAwesome(dataValues)}`];
    if (params.labels) queryParts.push(`chxl=0:|${formatLabels(parseLabels(params.labels))}`);
    ...
    if (params.lineWidth) queryParts.push(`chls=${params.lineWidth}`);
    queryParts.push(`chxt=x,y`);
